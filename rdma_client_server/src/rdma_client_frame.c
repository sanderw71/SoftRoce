...
//#sudo rdma link add rxe type rxe netdev eno2
...
static unsigned int src_len = 0, dst_len = 0;

/* This is our testing function */
static int check_src_dst() 
{
	for (int i = 0; i < strlen(src); i++) {
		printf("%c %c | ",src[i],dst[i]);
...
static int client_xchange_metadata_with_server()
{
	...
	client_send_sge.length = (uint32_t) htobe32(client_metadata_mr->length);
	client_send_sge.lkey = htobe32(client_metadata_mr->lkey);
 ...
 * purposes
 */
static int client_request_frames_from_server()
{
	struct ibv_wc wc[2];
	struct rdma_command rdma_cmd;
	int ret = -1;
	...
	/* we prepare metadata for the first buffer */
	client_metadata_attr.address = htobe64((uint64_t) client_src_mr->addr); 
	client_metadata_attr.length = htobe32((uint32_t) client_src_mr->length); 
	client_metadata_attr.stag.local_stag = htobe32(client_src_mr->lkey);
    ...
    if (client_send_sge.length == 0) {return -1;}
	/* now we link to the send work request */
	bzero(&client_send_wr, sizeof(client_send_wr));
	client_send_wr.sg_list = &client_send_sge;
	client_send_wr.num_sge = 1;
...
void usage() {
	printf("Usage:\n");
	printf("rdma_client_frame :[-a <server_addr>] [-p <server_port>] -s string (required)\n");
...
/*
	ret = client_request_frames_from_server(int size);
	if (ret) {
		rdma_error("Failed to setup client connection , ret = %d \n", ret);
		return ret;
	}
*/
	printf("client_request_frames_from_server\n\r");
	ret = client_request_frames_from_server();
	if (ret) {
		rdma_error("Failed to setup client connection , ret = %d \n", ret);
		return ret;
	}
