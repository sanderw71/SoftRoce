...
static int send_server_frame_to_client()
{
	struct ibv_wc wc;
	int ret = -1;
	...
	show_rdma_buffer_attr(&client_metadata_attr);
	printf("The client has requested buffer length of : %u bytes \n", 
			htobe32(client_metadata_attr.length));

    if (client_metadata_attr.length == 0) return -10;
	...
	server_send_wr.wr.rdma.rkey = htobe32(client_metadata_attr.stag.local_stag);
	server_send_wr.wr.rdma.remote_addr = htobe64((uint64_t)client_metadata_attr.address); 
...
int main(int argc, char **argv) 
{
	...
	ret = send_server_frame_to_client();
	if (ret) {
